...
class TritonAttentionImpl(AttentionImpl):
    ...
    def __init__(
        ...
        self.xdb_flash_prefill_enabled = _env_flag("XDB_R31_FLASH_ATTN_PREFILL")
        self.xdb_flash_prefill_min_q = int(
            os.environ.get("XDB_R31_FLASH_ATTN_MIN_Q", "2")
        )
        if self.xdb_flash_prefill_enabled:
            logger.info_once(
                "XDB_R31_FLASH_ATTN_PREFILL enabled: using flash-attn varlen "
                "for eligible decoder prefill attention."
            )

    @staticmethod
    def _pack_paged_kv_for_flash(
        ...
        token_idx = torch.arange(
            max_seqlen_k, device=key_cache.device, dtype=torch.long
        )
        valid = token_idx.unsqueeze(0) < seqused_k.to(torch.long).unsqueeze(1)
        logical_block = (token_idx // block_size).unsqueeze(0).expand(num_seqs, -1)
        physical_block = block_table.gather(1, logical_block).to(torch.long)
        physical_block = torch.where(
            valid, physical_block, torch.zeros_like(physical_block)
        )
        block_offset = (token_idx % block_size).unsqueeze(0).expand(num_seqs, -1)

        packed_k = key_cache[physical_block, block_offset][valid].contiguous()
        packed_v = value_cache[physical_block, block_offset][valid].contiguous()

        cu_seqlens_k = torch.empty(
            (num_seqs + 1,), device=key_cache.device, dtype=torch.int32
        )
        cu_seqlens_k[0] = 0
        cu_seqlens_k[1:] = torch.cumsum(seqused_k.to(torch.int32), dim=0)
        return packed_k, packed_v, cu_seqlens_k

    def _can_use_xdb_flash_prefill(
        self,
        query: torch.Tensor,
        key_cache: torch.Tensor,
        value_cache: torch.Tensor,
        ...
        if not self.xdb_flash_prefill_enabled:
            return False
        return (
            max_seqlen_q >= self.xdb_flash_prefill_min_q
            ...
            and query.dtype == key_cache.dtype
            and query.dtype == value_cache.dtype
            and query.shape[-1] <= 256
            and query.shape[-1] % 64 == 0
        )

    def _forward_xdb_flash_prefill(
        self,
        query: torch.Tensor,
        key_cache: torch.Tensor,
        value_cache: torch.Tensor,
        ...
        from flash_attn.flash_attn_interface import vllm_flash_attn_varlen_func

        packed_k, packed_v, cu_seqlens_k = self._pack_paged_kv_for_flash(
            key_cache, value_cache, seqused_k, block_table, max_seqlen_k
        )
    ...
    def forward(
        ...
        if self._can_use_xdb_flash_prefill(
            query,
            key_cache,
            value_cache,
            max_seqlen_q,
            ...
            self._forward_xdb_flash_prefill(
                query,
                key_cache,
                value_cache,
                attn_output,
