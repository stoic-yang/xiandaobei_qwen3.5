...
RUN_ID=runai-startup-probe-locked-20260707-1324
...
echo "=== runai locked startup probe ==="
echo "run_id=$RUN_ID"
echo "host=$(hostname)"
...
echo "cache_state=warm_after_prior_invalid_probe"
...
cat > "$RUN_DIR/start_vllm_runai_locked.sh" <<'EOS'
#!/usr/bin/env bash
set -u
set -o pipefail
...
EOS
chmod +x "$RUN_DIR/start_vllm_runai_locked.sh"

start_epoch=$(date +%s)
echo "server_start_at=$(date -Is) epoch=$start_epoch"
(cd "$RUN_DIR" && nohup bash "$RUN_DIR/start_vllm_runai_locked.sh" > "$RUN_DIR/vllm_server.log" 2>&1 & echo $! > "$RUN_DIR/vllm_server.pid")
pid=$(cat "$RUN_DIR/vllm_server.pid")
echo "server_pid=$pid"

health_ok() {
  curl -fsS --noproxy '*' http://127.0.0.1:8001/health >/dev/null 2>&1
}
...
patterns = {
    "stream_to_cpu_s": r"Overall time to stream [^:]+: ([0-9.]+)s",
    "stream_to_cpu_mib_s": r"Overall time to stream [^:]+: [0-9.]+s, ([0-9.]+) MiB/s",
    ...
    "torch_compile_total_s": r"torch\\.compile took ([0-9.]+) s in total",
    "graph_compile_1_4096_s": r"Compiling a graph for compile range \\(1, 4096\\) takes ([0-9.]+) s",
}
metrics = {}
...
summary = {
    ...
    "cache_state": "warm_after_prior_invalid_probe",
