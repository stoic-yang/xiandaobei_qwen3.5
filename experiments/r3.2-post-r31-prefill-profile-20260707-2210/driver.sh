#!/usr/bin/env bash
set -euo pipefail
RUN_ID=r3.2-post-r31-prefill-profile-20260707-2210
...
nohup hipprof --hip-trace --hsa-trace --trace-off --session "$SESSION" --flush-interval 1000 --buffer-size 5000 --output-type 0 -o "$RUN_DIR/profile/vllm_prefill" "$RUN_DIR/start_vllm_locked.sh" > "$RUN_DIR/profile/hipprof_service.log" 2>&1 &
...
cat > "$RUN_DIR/prefill_request.py" <<"PY3"
...
python3 "$RUN_DIR/prefill_request.py" > "$RUN_DIR/profile/prefill_request.stdout" 2> "$RUN_DIR/profile/prefill_request.stderr" || echo "prefill_request_failed=$?"
...
sleep 2
pkill -KILL -f "vllm serve .*Qwen3.5-27B" 2>/dev/null || true
if [ -f "$RUN_DIR/profile/vllm_prefill.db" ]; then
  hipprof --db "$RUN_DIR/profile/vllm_prefill.db" --output-type 0 -o "$RUN_DIR/profile/vllm_prefill_export" > "$RUN_DIR/profile/hipprof_export.log" 2>&1 || true
fi
