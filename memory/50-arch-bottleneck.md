...
- 2026-07-09 R4.1 16-32K prefill roofline/profile（`experiments/r4.1-prefill-16to32-profile-20260709-1120/`, locked `max_seq_len=32768`, `load_format=runai_streamer`, R3.1 flash path on）：first 16-32K prompt API usage `prompt_tokens=20576`, request wall `7169.049ms`, hipkernel total `7053.787ms`. `Cijk_*` is `3154.114ms` / `44.715%`, aggregate `317.396 TFLOPS` = `80.353%` of external `395 TFLOPS` peak; flash-attn `_fwd_kernel` is `2862.586ms` / `40.582%`; GDN core+helpers `610.613ms` / `8.657%`. Verdict: GEMM remains yellow-zone and direct GEMM kernel/INT8 implementation is not yet justified; new higher-value growth point is long-context flash-attn R3.1b diagnostic/tuning (`plans/task-r3.1b-long-context-flash-attn.md`). Post-run evidence: no vLLM/hipprof process left, ports `18001`/`8001` down, `hy-smi` HCU and memory use `0%`.
- 2026-07-09 R3.1b Step0 source attribution（`experiments/r3.1b-step0-fused-prefill-source-20260709/`）：R4.1 source head was `4653a56d5`, whose `triton_attn.py` checks `VLLM_TRITON_FUSED_PREFILL` before `XDB_R31_FLASH_ATTN_PREFILL`; the `16-32K` `_fwd_kernel` (`96` calls, `2862.586ms`, `40.582%`) matches local `vllm/v1/attention/ops/triton_flash_prefill.py::_fwd_kernel`, introduced by `f014885bc`, not package `flash_attn.flash_attn_interface.vllm_flash_attn_varlen_func`. Next A/B should first compare fused-on current default vs `VLLM_TRITON_FUSED_PREFILL=0` xdb varlen vs unified fallback; only test `FLASH_ATTENTION_TRITON_AMD_AUTOTUNE` if xdb/package varlen is competitive.
- 2026-07-09 R3.1b fused-on/off A/B（`experiments/r3.1b-fused-ab-20260709/`, container `Instances_2607090940238205_1_0` / job `662724`）：locked `runai_streamer`, isolated `18001`, 16-32K `3` prompts x `3` reps. A current fused default (`VLLM_TRITON_FUSED_PREFILL=1`) median output `4.122288 tok/s`, TTFT-P99 `21793.173ms`, TPOT-P99 `71.514ms`; B xdb/package varlen (`VLLM_TRITON_FUSED_PREFILL=0`) median output `4.297041 tok/s`, TTFT-P99 `19279.477ms`, TPOT-P99 `71.532ms`. B is `+4.239%` output and `-11.535%` TTFT on long bucket, proving a long-context routing opportunity; TPOT unchanged, so gain is prefill-side.
- 2026-07-09 R3.1b global varlen stop-loss（`experiments/r3.1b-xdbvarlen-3bucket-20260709-1700/`）：global `VLLM_TRITON_FUSED_PREFILL=0` three-bucket medians were `8.021708 / 8.175304 / 4.294348 tok/s` for `4-8K / 8-16K / 16-32K`, with logs showing prompt throughput near old `~758 tok/s`. Verdict: do **not** globally disable fused prefill; use a threshold/hybrid dispatch if pursuing the `16-32K` gain.
- 2026-07-09 R3.1b threshold candidate prepared but not validated（remote isolated worktree `/public/home/xdzs2026_c166/vllm_cscc_codex_r31b_threshold_20260709_173633`, commit `b82c6a4a6fef63c276b982194f8baee3bd91a7f9`, patch `experiments/r3.1b-fused-ab-20260709/threshold-varlen.patch`）：adds default-off `VLLM_TRITON_FUSED_PREFILL_MAX_SEQ_LEN`; default `0` is current behavior, `16384` keeps short/mid on fused and routes long prefill to xdb varlen. Validation run `experiments/r3.1b-threshold16384-3bucket-20260709-1739/` was blocked at startup, not performance-failed: a separate `Qwen3.5-0.8B` service on port `8001` left only `59.54/63.98 GiB` free, below locked `gpu_memory_utilization=0.95` requirement `60.79 GiB`. Do not clean that service without owner consent; next clean 27B container should run this threshold candidate first.
