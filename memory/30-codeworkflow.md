...
- 2026-07-07 Codex startup method note: `runai_streamer` startup probes are infrastructure measurements, not R1 commit sign rows. `experiments/runai-startup-probe-fresh-20260707-1259/` is invalid for competition timing because the user-provided script omitted locked `--max-model-len 32768` and vLLM resolved `max_seq_len=262144`; `experiments/runai-startup-probe-locked-20260707-1324/` restored locked params and reached `/health` in `725s` from `vllm serve`, with RunAI streaming `51.7 GiB` in `341.16s` and engine init after load `279.43s`. Use this path to reduce container rebuild tax when `/root/Qwen3.5-27B` is absent, but keep guard throughput A/B on `scripts/guard_bench.py` protocol.
- 2026-07-07 Codex guard口径更正：远端 `testdata/start_vllm.sh` 当前没有 `--max-model-len 32768`；已归档的旧 guard 日志中 `experiments/guard-a55f3c3-overlay-8to16-20260706-2252/vllm_server.log` 与 `experiments/guard-d29e9db3-20260706-2005/vllm_server.log` 均显示 `Using max model len 262144` / `max_seq_len=262144`。历史 guard 的同口径相对符号可作筛查参考，但不满足锁定 CLI 红线；正式 R1 sign table 必须用 `scripts/guard_bench.py --locked-start-script --load-format runai_streamer` 重新跑，并在 `vllm_server.log` 验证 `max_seq_len=32768`。脚本已新增该显式开关，默认旧行为不变。
- 2026-07-07 Codex pivot: user asked to pause remaining R1 commit A/B because likely regressions are small and higher-ROI work should come first. Running locked d29 guard `experiments/guard-d29e9db3-locked-srcdir-fullsmoke10-20260707-1355/` was stopped during 8-16K rep1; it is **not** a sign-table row. It did validate the corrected locked startup path (`max_seq_len=32768`, `runai_streamer`) and completed 4-8K reps with median `12.256351` output tok/s. Earlier attempts `...1344` and `...1351` are invalid tool-path experiments: shared-git overlay stall, then heredoc dedent bug fixed by `90459ea`.
- 2026-07-07 Codex R2.1 tooling: `scripts/guard_bench.py` gained diagnostic `--enforce-eager` support, gated behind `--locked-start-script`, and now collects `vllm_server.log` plus `start_vllm_locked.sh` by default for locked CLI evidence. This was used by `experiments/r2-eager-enforce-ab-20260707/`; keep it diagnostic-only unless a task card explicitly asks for an eager comparison.
