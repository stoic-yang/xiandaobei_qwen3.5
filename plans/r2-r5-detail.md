...
- **⚠️ 审计警告：FLA GDN 是"更 risky 的一半"，不是库白拿**。FLA chunked-scan 是 CUDA 假设很重的 Triton，在 gfx936 Triton 上的可移植性+性能是**真未知数**；~30% 占比接近 full-attn，**给它独立 spike + 一等风险位**，别当脚注。full-attn 有两个成熟 ROCm FA 后端兜底，GDN 没有。
...
## 建议执行顺序（2026-07-07 更新：R2.0/R2.1 已诊断关闭，主力转 R3.2）
...
- **现在推进 `task-r3.2-gdn-prefill.md`**：先跑 post-R3.1 prefill profile + shape dump，再做 GDN config/autotune spike。
- 之后：R2.4/R3.0（GEMM 库，若 post-R3.1 profile 指向 GEMM）→ R3.2 GDN 候选 A/B → R4（可选 INT8 prefill 增强）→ R5 收尾。
